...
func RenderTemplate(w http.ResponseWriter, r *http.Request, tmpl string, data interface{}, block interface{}) {
	...
	t, err := template.New("base.html.tmpl").Funcs(funcs).ParseFiles("web/templates/base.html.tmpl", "web/templates/"+tmplLang)
	if err != nil {
	...
	user, err := services.GetSessionUser(r)
	if err == services.ErrSessionCookieNotFound {
		log.Printf("Session cookie not found")
		newdata["User"] = nil
	} else if err == db.ErrSessionNotFound {
		log.Printf("Session not found")
		newdata["User"] = nil
	} else if err != nil {
		log.Printf("Error getting session user: %v", err)
		http.Error(w, "Failed to get session user", http.StatusInternalServerError)
		return
	} else {
		newdata["User"] = user
	}
